...
function patchHolderViewIfChanged(holder, view) {
  if (view !== holder["nsView"]._subViews[0]) {
    console.log("patchHolderViewIfChanged", view);
...
export class CollectionView extends CollectionViewBase {
  ...
  public createNativeView() {
    ...
    const recyclerView = new CollectionViewRecyclerView(
      this._context,
      new WeakRef(this)
    );
    ...
    recyclerView.setDescendantFocusability(
    ...
    const layoutManager = (this.layoutManager = new GridLayoutManager(
      this._context,
      new WeakRef(this)
    ));
  ...
  _getViewLayoutParams() {
    ...
    layoutParams.width =
      org.nativescript.widgets.CommonLayoutParams.MATCH_PARENT;
    layoutParams.height =
      org.nativescript.widgets.CommonLayoutParams.WRAP_CONTENT;
    // }
      ...
      case "splice":
        {
          if (event.addedCount > 0) {
            this._listViewAdapter.notifyItemRangeInserted(
              event.index,
              event.addedCount
            );
            return;
          ...
          if (event.removed && event.removed.length > 0) {
            this._listViewAdapter.notifyItemRangeRemoved(
              event.index,
  ...
  public refresh() {
    ...
    if (this._listViewAdapter) {
      ...
      this._listViewAdapter = this.createComposedAdapter(
    ...
    if (this.isHorizontal()) {
      if (this._effectiveRowHeight) {
        spanCount =
          Math.max(
            Math.floor(this._innerHeight / this._effectiveRowHeight),
            1
          ) || 1;
      }
    } else {
...
interface CollectionViewScrollListener
...
function initCollectionViewScrollListener() {
  ...
  class CollectionViewScrollListenerImpl extends android.support.v7.widget
    ...
    public onScrolled(
      ...
      if (
        owner.hasListeners(CollectionViewBase.loadMoreItemsEvent) &&
        owner.items
      ) {
...
interface CollectionViewAdapter
  extends android.support.v7.widget.RecyclerView.Adapter<any> {
  // tslint:disable-next-line:no-misused-new
...
function initCollectionViewAdapter() {
  ...
  class CollectionViewCellHolder
    extends android.support.v7.widget.RecyclerView.ViewHolder
    implements android.view.View.OnClickListener {
    constructor(
      private owner: WeakRef<View>,
      private gridView: WeakRef<CollectionView>
    ) {
  ...
  > {
    constructor(
      private owner: WeakRef<CollectionView>,
    ...
    public getItemCount() {
      ...
      // console.log('getItemCount',  owner.items ? owner.items.length : 0);
    ...
    public getItemViewType(position: number) {
      ...
      // console.log("getItemViewType", position, !!owner.itemTemplateSelector);
      if (owner.itemTemplateSelector) {
        ...
        if (dataItem) {
          ...
          if (!this.templateTypeNumberString.has(selectorType)) {
            this.templateTypeNumberString.set(
              selectorType,
    ...
    public onCreateViewHolder(
      ...
      const view = owner.getViewForViewType(
        ListViewViewTypes.ItemView,
        templateType
      );

      // console.log("onCreateViewHolder", templateType, view);

      var parentView = new StackLayout();
      // parentView.backgroundColor = "red";
      // parentView.padding = 10;
      // parentView.borderRadius = 10;
      var layoutParams = owner._getViewLayoutParams();
      parentView.orientation = "vertical";
      parentView.addChild(view);
      this._viewHolderChildren.push(parentView);

      propagateInheritableProperties(owner, parentView);
      (parentView as any)._inheritStyleScope((owner as any)._styleScope);
      propagateInheritableCssProperties(owner.style, parentView.style);
      parentView._setupUI(parent.getContext());
      if (!parentView.isLoaded) {
        (parentView as any).callLoaded();
      }

      parentView.nativeView.setLayoutParams(layoutParams);
      // parentView.nativeView.setBackgroundResource(this._selectionViewId);
      var holder = new CollectionViewCellHolder(
        new WeakRef(parentView),
        new WeakRef(owner)
      );
      holder["nsView"] = parentView;
      this._viewHolders.push(holder);

      // console.log('onCreateViewHolder2', template.key, viewType, !!view._context, !!view.parent, !!view.nativeView);
      // console.log('onCreateViewHolder', Date.now() - date + 'ms');
      // owner._addView(view);

      return holder;

      // const v = android.view.LayoutInflater.from(parent.getContext()).inflate(getLayout('text_row_item'), parent, false);
    ...
    // public onCreateHeaderItemViewHolder(parent: android.view.ViewGroup, viewType: number) {
    //     return null;
    // }
    // public onCreateFooterItemViewHolder(parent: android.view.ViewGroup, viewType: number) {
    //     return null;
    // }
    ...
    public onBindViewHolder(
      ...
      // console.log("onBindViewHolder", data);
      holder["nsView"].bindingContext = data;
      let args = {
      ...
      if (owner._effectiveColWidth || !holder.view.width) {
        holder.view.width = utils.layout.toDeviceIndependentPixels(
          owner._effectiveColWidth
        );
      }
      if (owner._effectiveRowHeight || !holder.view.height) {
        holder.view.height = utils.layout.toDeviceIndependentPixels(
          owner._effectiveRowHeight
        );
      }

      // owner._prepareItem(vh.view, index);
      // this.notifyForItemAtIndex(owner, vh, vh.view, CollectionViewBase.itemLoadingEvent, index);
      // vh.getTextView().setText(data.value);
    }
    // public onBindHeaderItemViewHolder(vh: CollectionViewCellHolder, index: number) {
    //     super.onBindHeaderItemViewHolder(vh, index);
    // }
    // public onBindFooterItemViewHolder(vh: CollectionViewCellHolder, index: number) {
    //     super.onBindFooterItemViewHolder(vh, index);
    // }
    // public getItemCount() {
    //     const owner = this.owner.get();
    ...
    public onClick(v: android.view.View) {
      const rv = com.h6ah4i.android.widget.advrecyclerview.utils.RecyclerViewAdapterUtils.getParentRecyclerView(
        v
      );
      ...
      const localPosition = com.h6ah4i.android.widget.advrecyclerview.utils.WrapperAdapterUtils.unwrapPosition(
        rootAdapter,
        this,
        rootPosition
      );
      ...
      // if (segment === CollectionViewAdapterImpl.SEGMENT_TYPE_HEADER) {
      //     // Header item is clicked !
      // } else if (segment === CollectionViewAdapterImpl.SEGMENT_TYPE_FOOTER) {
...
export interface CollectionViewRecyclerView
...
function initCollectionViewRecyclerView() {
  ...
  class CollectionViewRecyclerViewImpl extends android.support.v7.widget
    .RecyclerView {
    constructor(
...
function initGridLayoutManager() {
  ...
  class GridLayoutManagerImpl extends android.support.v7.widget
    .GridLayoutManager {
