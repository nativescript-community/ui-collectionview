...
<page>
    ...
    <gridLayout>
        <collectionView colWidth="50%" {items} layoutStyle="waterfall" on:itemTap={onItemTap} on:loadMoreItems={onLoadMoreItems}>
            <Template let:item>
                <gridlayout class="item" backgroundColor={item.color} height={randomHeight(item.color)} rows="*, auto">
                    <stacklayout row="1">
